...
namespace EpipolarConsistency
{
	...
	double estimateObjectRadius(const ProjectionMatrix& P, int n_u, int n_v)
	{
		...
		// Compute field of view (use minimum of u and v).
		double fov   =min(abs(atan(0.5*n_u/alphas.first)), abs(atan(0.5*n_v/alphas.second)));
	...
	Metric& Metric::setEpipolarPlaneNumber(int _num_planes) {
	...
	Metric::Metric() : object_radius_mm(0), num_planes(0), n_u(0), n_v(0) {}
