...
namespace EpipolarConsistency
{
	...
	protected:
		/// Number of epipolar planes during sampling. Zero for automatic.
		int num_planes; 
	...
	public:
		...
		/// Manually set number of epipolar planes during sampling. Zero for automatic determination per view pair (recommended).
		Metric& setEpipolarPlaneNumber(int num_planes=0);
		...
		virtual double evaluateForImagePair(int i, int j,
			...
			std::vector<float> *kappas=0x0, double *weight=0x0) =0;
