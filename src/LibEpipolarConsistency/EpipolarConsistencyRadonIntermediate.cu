...
__global__ void kernelEpipolarConsistencyComputeK01(
		...
		int	num_planes		//< Number of planes to be sampled
		)
{
...
__global__ void kernelEpipolarCosistency(
		...
		float *weights				//< Weights for each pair
	)
{
	// Find index of current thread
	...
	if (idx_y==0)
	{
		...
		weights[idx_x]=1.0f-weighting((fabs(K0[7]-Pi)-no_contrib)/full_contrib);
	}
...
void epipolarConsistency(
	...
	int num_planes,                   //< Number of epipolar planes (can be zero for automatic)
	bool isDerivative,		//< If false, normal Radon transform, else its derivative
	float *weights_d		//< Temporary memory for weights computation
	)
{
	cudaCheckState
	...
	int actual_num_planes = num_planes;
	if (actual_num_planes <= 0)
			actual_num_planes = (int)std::round(image_diagonal);
	...
	int max_num_samples = actual_num_planes / 2;
	block_size.x=4;
	block_size.y=256;
	if (indices_d)
		grid_size.x=iDivUp(num_pairs,block_size.x);
	...
	{
		if (isDerivative)
			kernelEpipolarCosistency<true><<<grid_size, block_size>>>( K01s_d, (cudaTextureObject_t *)dtrs_d, n_t*step_t, num_pairs, indices_d, out_d, weights_d );
		else
			kernelEpipolarCosistency<false><<<grid_size, block_size>>>( K01s_d, (cudaTextureObject_t *)dtrs_d, n_t*step_t, num_pairs, indices_d, out_d, weights_d );
	}
	else
	{
		if (isDerivative)
			kernelEpipolarCosistency<true><<<grid_size, block_size>>>( K01s_d, num_dtrs, (cudaTextureObject_t *)dtrs_d, n_t*step_t, out_d, weights_d );
		else
			kernelEpipolarCosistency<false><<<grid_size, block_size>>>( K01s_d, num_dtrs, (cudaTextureObject_t *)dtrs_d, n_t*step_t, out_d, weights_d );
	}
	cudaDeviceSynchronize();
	cudaCheckState
}
