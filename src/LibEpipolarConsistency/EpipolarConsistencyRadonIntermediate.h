...
namespace EpipolarConsistency
{
	...
	public:
		...
		{
			num_planes = _dkappa > 0 ? (int)std::round(3.14159265359f * 0.5f / _dkappa) : 0;
			return *this;
		}
		...
		virtual double evaluateForImagePair(int i, int j,
			...
			std::vector<float> *kappas=0x0, double *weight=0x0);
	...
	protected:
		...
		UtilsCuda::MemoryBlock<float>	*weights;	//< Weights for each view pair
