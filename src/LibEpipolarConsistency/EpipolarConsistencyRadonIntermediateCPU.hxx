...
namespace EpipolarConsistency
{
    ...
    struct MetricCPU {
            ...
            double                                                          object_radius_mm; //< Object radius in mm
            ...
            MetricCPU(
                    ...
                    int _num_planes=0,
                    double _object_radius_mm=0.0
                    )
                    : Ps(_Ps)
                    , dtrs(_dtrs)
                    , num_planes(_num_planes)
                    , object_radius_mm(_object_radius_mm)
            {}
		...
		inline double operator()()
		{
			...
			for (int i=0;i<n;i++)
				...
				for (int r=i+1;r<n;r++)
                                    sum+=computeForImagePair(Cs[r], Cs[i], Ps[r], Ps[i], dtrs[r], dtrs[i], num_planes, object_radius_mm);
		...
		/// Compute geometric consitency metric for one pair of views.
            ...
            static double computeForImagePair(
                    ...
                    int                                                    num_planes,             //< Number of planes
                    double                                                 object_radius_mm=0.0,   //< Object radius in mm
                    ...
                    )
            {
                    ...
                    double r_mm = object_radius_mm;
                    if (r_mm <= 0.0) {
                            r_mm = std::max(
                                    estimateObjectRadius(P0, dtr0->getOriginalImageSize(0), dtr0->getOriginalImageSize(1)),
                                    estimateObjectRadius(P1, dtr1->getOriginalImageSize(0), dtr1->getOriginalImageSize(1))
                            );
                    }
                    ...
                    float n_t=(float)dtr0->getRadonBinNumber(1);
                    float n_alpha=(float)dtr0->getRadonBinNumber(0);
                    ...
                    float sum=0;
                    int nsamples=0;
                    for (int idx_y = 0; idx_y < num_planes / 2; ++idx_y)
                    {
                            float kappa = dkappa * 0.5f + dkappa * idx_y;
                            if (kappa >= kappa_max) break;
                            ...
                            float px_alpha0, px_t0, px_alpha1, px_t1;
                            ...
                            // Return consistency
                            float consistency = ((vp0-vp1)*(vp0-vp1) + (vm0-vm1)*(vm0-vm1)) * K0[6];
                            ...
                            if (debug_out)
                            {
                                    ...
                                    samples0->push_back(vm0);
                                    samples1->push_back(vm1);
                                    angles->push_back(-kappa/Pi*180.);
                            }
                    }
                    ...
                    if (weight)
                    {
                            ...
                            float no_contrib = 0.05f * Pi;
                            float full_contrib = 0.2f * Pi;
                            float x = (fabs(K0[7] - Pi) - no_contrib) / full_contrib;
                            *weight = (double)(1.0f - weighting(x));
                    }
