...
namespace EpipolarConsistency
{
	MetricRadonIntermediate::MetricRadonIntermediate()
		...
		, weights(new UtilsCuda::MemoryBlock<float>())
	{}
	...
	double MetricRadonIntermediate::evaluate(float * _out)
	{
		...
		epipolarConsistency(
			...
			(float)getObjectRadius(), num_planes, m_isDerivative, *weights);

		// Readback intermediate data
		NRRD::Image<float> out_sums(1,n*(n-1)/2);
		weights->readback(out_sums);
		
		// Read back and sum up
		out->readback(_out);

		// Compute weighted sums of SSD
	...
	double MetricRadonIntermediate::evaluate(const std::vector<Eigen::Vector4i>& _indices, float * _out)
	{
		...
		epipolarConsistency(
			...
			(float)getObjectRadius(), num_planes, m_isDerivative,
			*weights);

		// Read back intermediate results
		NRRD::Image<float> out_sums(1,n_pairs);
		weights->readback(out_sums);
		out->readback(_out);
	...
	double MetricRadonIntermediate::evaluateForImagePair(int i, int j,
		...
		std::vector<float> *kappas, double *weight)
	{
		return evaluateForImagePair(i,j,redundant_samples0,redundant_samples1,kappas,0x0,0x0,weight);
	}

	double MetricRadonIntermediate::evaluateForImagePair(int i, int j,
		std::vector<float> *redundant_samples0, std::vector<float> *redundant_samples1,
		std::vector<float> *kappas, std::vector<std::pair<float,float> > *radon_samples0, std::vector<std::pair<float,float> > *radon_samples1, double *weight )
	{
		// Epipolar Geometry
		...
		int np = num_planes;
		if (np <= 0)
			np = (int)std::round(sqrtf((float)(n_u*n_u+n_v*n_v)));
		float K0[8], K1[8];
		computeK01(
			n_u*0.5f,n_v*0.5f,
			C0f.data(),C1f.data(),P0invTf.data(),P1invTf.data(),
			(float)getObjectRadius(),np,
			K0,K1);
		if (weight)
		{
			const float Pi=3.14159265359f;
			const float no_contrib=0.05f*Pi, full_contrib=0.2f*Pi;
			*weight = 1.0f-weighting((fabs(K0[7]-Pi)-no_contrib)/full_contrib);
		}
