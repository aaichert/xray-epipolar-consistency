...
namespace UtilsCuda
{
	...
	BindlessTexture1D<Type>::BindlessTexture1D(int w, const Type * buffer, bool buffer_is_device, bool interpolate, bool normalizedCoords)
		: normalizedCoords(normalizedCoords)
	{
		cudaCheckState
		size[0]=w;
		cudaChannelFormatDesc channelDesc=cudaCreateChannelDesc<Type>();
		cudaMallocArray(&array, &channelDesc, w, 0);
		cudaCheckState
		cudaMemcpyToArray(array, 0, 0, buffer, sizeof(Type)*w, buffer_is_device ? cudaMemcpyDeviceToDevice : cudaMemcpyHostToDevice);
		cudaCheckState
	...
	template <typename Type>
	BindlessTexture1D<Type>::~BindlessTexture1D()
	{
		cudaFreeArray(array);
	...
	template class BindlessTexture1D<float>;
	template class BindlessTexture1D<float4>;
