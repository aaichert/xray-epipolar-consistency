	...
	)
{
	...
	for (float a=0;a<ray_length;a+=1.0/samples_per_voxel)
	{
		...
		if (sample>iso_val)
		{
			...
			float gradient[]={
				(get_sample_clipped(voxel_data,voxel[0]+1.0f,voxel[1],voxel[2],n_planes,model_C_Pinv_h)-get_sample_clipped(voxel_data,voxel[0]-1.0f,voxel[1],voxel[2],n_planes,model_C_Pinv_h)),
				(get_sample_clipped(voxel_data,voxel[0],voxel[1]+1.0f,voxel[2],n_planes,model_C_Pinv_h)-get_sample_clipped(voxel_data,voxel[0],voxel[1]-1.0f,voxel[2],n_planes,model_C_Pinv_h)),
				(get_sample_clipped(voxel_data,voxel[0],voxel[1],voxel[2]+1.0f,n_planes,model_C_Pinv_h)-get_sample_clipped(voxel_data,voxel[0],voxel[1],voxel[2]-1.0f,n_planes,model_C_Pinv_h))
			};
	...
	)
{
	...
	if (n_c==1) // shaded iso-surface
		...
		kernel_raycast_iso<6><<<grid_size, block_size>>>(n_u, n_v, pixel_data_d, model_C_Pinv_d, voxel_data, ray_entry_d, ray_exit_d, noise_d, samples_per_voxel, iso_val);
	else if (n_c==7) //  xyz surface point and xyz normal and shading
		kernel_raycast_iso<7><<<grid_size, block_size>>>(n_u, n_v, pixel_data_d, model_C_Pinv_d, voxel_data, ray_entry_d, ray_exit_d, noise_d, samples_per_voxel, iso_val);
